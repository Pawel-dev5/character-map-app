...
interface CharacterFormProps {
	...
	updateCharacter: (key: keyof Character, value: Character[keyof Character]) => void;
}

export const CharacterForm = ({ name, color, colorName, avatarType, updateCharacter }: CharacterFormProps) => {
	const { t } = useTranslation();
	const [localName, setLocalName] = useState(name);

	const { fetchColorName, loading, error, clearError, isNetworkError, isTimeoutError, isInvalidColorError } = useColorName();
			...
			<form className="character-form">
				<div className="form-field">
					...
					<input
						...
						aria-label={t('characterForm.namePlaceholder')}
						role="textbox"
					/>
				</div>
				...
				<div className="form-field">
					...
					<select
						...
						value={avatarType}
						onChange={(e) => updateCharacter('avatarType', e?.target?.value as AvatarType)}
						className="form-select avatar-select"
					>
						<option value="basic">{t('characterForm.avatarRogue', 'Basic')}</option>
						<option value="bit8">{t('characterForm.avatarWarrior', '8-bit Character')}</option>
						<option value="santa">{t('characterForm.avatarMage', 'Santa')}</option>
						<option value="car">{t('characterForm.avatarArcher', 'Car')}</option>
					</select>
				</div>
				...
				<div className="form-field">
					...
					<div className="color-input-container">
						...
						<span className={getColorInfoClass()}>
							...
							{colorName && !loading && !error && <span>{t('characterForm.colorSuccess', { colorName })}</span>}
							{!colorName && !loading && !error && (
								<span style={{ opacity: 0.6 }}>{t('characterForm.colorPlaceholder')}</span>
							)}
						</span>
					</div>
				</div>
