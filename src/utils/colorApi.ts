...
import type { ColorApiResponse, ColorNameResult, ApiError, ErrorType as ErrorTypeUnion } from '../types/api';
...
const getErrorMessage = (errorType: ErrorTypeUnion, status?: number): string => {
	switch (errorType) {
...
export const fetchColorName = async (hexColor: string): Promise<ColorNameResult> => {
	try {
		...
		const response = await axios.get<ColorApiResponse>(`https://www.thecolorapi.com/id`, {
			params: { hex: cleanHex },
			timeout: 5000,
		});
