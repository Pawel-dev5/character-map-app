...
export const useMap = () => {
	...
	const [currentCoordinates, setCurrentCoordinates] = useState<GeoCoordinates | null>(() => {
		return mapStorage.getCoordinates();
	});

	const updateMapConfig = useCallback(<K extends keyof MapConfig>(key: K, value: MapConfig[K]) => {
